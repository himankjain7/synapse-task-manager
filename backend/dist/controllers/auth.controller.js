...
class AuthController {
     ...
     * - 500: Server error
     */
    static signup = (0, error_middleware_1.asyncHandler)(async (req, res) => {
        const { email, password, name, avatarUrl } = req.body;
        // Call service
        const user = await auth_service_1.AuthService.signup(email, password, name, avatarUrl);
        // Response (201 Created)
        res.status(201).json({
            success: true,
            data: user,
     ...
     * }
     */
    static getGoogleLoginUrl = (0, error_middleware_1.asyncHandler)(async (req, res) => {
        ...
        // Generate Google login URL
        const url = GoogleAuthService_1.GoogleAuthService.getAuthURL(state);
        res.status(200).json({
            success: true,
            data: { url },
            timestamp: new Date(),
     ...
     */
    static googleCallback = (0, error_middleware_1.asyncHandler)(async (req, res) => {
        ...
        const result = await GoogleAuthService_1.GoogleAuthService.completeOAuthFlow(code);
    ...
    static updateProfile = (0, error_middleware_1.asyncHandler)(async (req, res) => {
        ...
        const user = await auth_service_1.AuthService.updateProfile(userId, name, avatarUrl);
    ...
    static changePassword = (0, error_middleware_1.asyncHandler)(async (req, res) => {
        ...
        if (!currentPassword || !newPassword) {
            res.status(400).json({ code: 'VALIDATION_ERROR', message: 'Current and new password required' });
            return;
        }
        if (newPassword.length < 8) {
            res.status(400).json({ code: 'VALIDATION_ERROR', message: 'New password must be at least 8 characters' });
            return;
        }
     ...
     * - 401: No valid token
     */
    static getMe = (0, error_middleware_1.asyncHandler)(async (req, res) => {
     ...
     * }
     */
    static verifyToken = (0, error_middleware_1.asyncHandler)(async (req, res) => {
        ...
        try {
            const payload = await auth_service_1.AuthService.verifyJWT(token);
            res.status(200).json({
                success: true,
                data: {
                    valid: true,
