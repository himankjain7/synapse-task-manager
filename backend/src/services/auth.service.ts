...
export class AuthService {
  ...
  static async signup(
    ...
    if (existingUser) {
      ...
      (error as any).code = ErrorCode.DUPLICATE_EMAIL;
      throw error;
    }
  ...
  static async hashPassword(password: string): Promise<string> {
    try {
      ...
      const hash = await bcrypt.hash(password, this.BCRYPT_SALT_ROUNDS);
      return hash;
    } catch (error) {
   ...
   * @private
   */
  private static hashTokenForBlacklist(token: string): string {
