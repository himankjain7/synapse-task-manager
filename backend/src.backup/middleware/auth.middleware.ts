...
import { verifyAccessToken, safeVerifyToken } from '../utils/jwt';
import { AuthContext, JWTPayload } from '../models';
...
export const optionalAuth = (
  ...
  try {
    ...
    // Try to verify token
    const result = safeVerifyToken(token, 'access');
 ...
 */
export const workspacePermission = (minRole: 'guest' | 'member' | 'admin' | 'owner') => {
 ...
 * @returns Middleware function
 */
export const rateLimit = (limit: string) => {
...
// Extend Request interface to include custom user payload
